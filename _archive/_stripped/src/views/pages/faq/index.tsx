  ...
  return (
    <section>
      <div className='space-y-6 md:space-y-8 lg:space-y-10'>
        {/* Header */}
        <div className='bg-card space-y-4 rounded-md px-6 py-12 text-center'>
              ...
              <TabsList className='h-max w-full flex-col gap-2 bg-transparent p-0'>
                {data.categories.map(category => (
                  <TabsTrigger
                    key={category.id}
                    value={category.id}
                    className='border-border bg-muted text-foreground data-active:border-primary/20 data-active:bg-card data-active:text-primary dark:data-active:border-primary/20 dark:data-active:bg-primary/10 dark:data-active:text-primary w-full gap-2 rounded-lg px-6 py-2.5 data-active:shadow-none!'
                  >
              ...
              {data.categories.map(category => (
                <TabsContent key={category.id} value={category.id} className='mt-0 space-y-4'>
                  <div className='flex min-h-10 items-center justify-start gap-4'>
                    <Avatar size='lg' className='bg-card rounded-md border shadow-none after:border-0'>
                      <AvatarFallback className='text-foreground bg-card rounded-md [&>svg]:size-5'>
                        {faqIconMap[category.icon]}
                      </AvatarFallback>
                    </Avatar>
                    <div>
