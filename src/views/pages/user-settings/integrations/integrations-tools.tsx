...
const IntegrationsTools = ({ apps }: { apps: IntegrationApp[] }) => {
  ...
  return (
    <Card className='grid grid-cols-1 gap-10 lg:grid-cols-3'>
      ...
      <CardContent className='flex flex-col space-y-1'>
        ...
        <p className='text-muted-foreground text-sm'>Manage your tool integrations and settings.</p>
      </CardContent>
      {/* Content */}
        ...
        <div className='grid gap-6 sm:grid-cols-2 xl:grid-cols-3'>
          {apps.map((app, index) => (
            <Card key={index} className='group hover:ring-primary transition-colors'>
              ...
              <Dialog>
                <CardContent>
                  <DialogTrigger render={<Button variant='outline' className='h-7 w-full gap-1.5 px-2 py-1 text-xs' />}>
                    View Integration
                    <ChevronRightIcon className='transition-transform group-hover:translate-x-1' />
                  </DialogTrigger>
                </CardContent>
                <DialogContent className='max-w-[95%] gap-0 overflow-hidden p-0 max-lg:h-[70%] lg:max-w-3xl'>
                  {/* Header */}
                  ...
                  <div className='flex overflow-hidden max-sm:flex-col'>
                    ...
                    <div className='max-h-[60vh] flex-1 space-y-5 overflow-y-auto p-4 md:p-6 lg:max-h-[65vh]'>
                      <p className='text-muted-foreground text-sm'>
                        Seamlessly bridge the gap between your support and engineering teams. This integration connects
                        your ticketing system with {app.name}, enabling real-time syncing and powerful automation
                        capabilities.
                      </p>
                    ...
                    <div className='flex w-52 shrink-0 flex-col gap-5 border-l p-5 max-sm:w-full'>
                      ...
                      <div className='max-sm:hidden'>
                        ...
                        <div className='flex flex-wrap gap-1.5'>
                          {['Conversation management', 'Issue tracking & ticketing', 'For Support Agents'].map(tag => (
