...
1. **Base UI only, decided per item.** There are zero `@radix-ui/*` packages
   today and that must not change — but do not assume Studio is Radix. This repo
   is on `style: base-vega`, so the registry serves its Base UI variant. Resolve
   the item, read it, and apply the gate in *Shadcn Studio frontend authority*
